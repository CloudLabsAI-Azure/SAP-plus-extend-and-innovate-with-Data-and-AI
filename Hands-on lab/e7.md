...
1. In Synapse Studio, open the **Manage (1)** hub from the left menu, choose **SQL pools (2)** from the center menu and select the **sapdatasynsql (3)** dedicated SQL pool from the list.
...
2. On the Get Data dialog, select **Azure (1)** from the left menu and select **Azure Synapse Analytics SQL (2)** from the listing. Select the **Connect (3)** button.
...
4. An authentication dialog displays, select **Microsoft account (1)** from the left menu, then select the **Sign in (2)** button. Authenticate with your Azure credentials.
...
7. The columns in the tables representing sales order numbers were incorrectly interpreted as strings by Power BI. In the Power Query editor screen, select **SalesOrderItems (1)** from the Queries pane, then right-click on the **SalesOrder (2)** field and expand the **Change Type (3)** item and choose **Whole Number (4)**.
...
1. In Power BI Desktop, expand the **Transform data (1)** item on Home tab toolbar menu. Select **Transform data (2)**.
...
3. In the Merge window, select the **SALESDOCUMENT (1)** column of the **SalesOrderHeaders** table. Select the **Payments (2)** table as the second table and select the **SalesOrderNr (3)** field. Select **OK (4)**.
...
7. From the report designer, expand the **Transform data (1)** item on Home tab toolbar menu. Select **Transform data (2)**.

   ![The Home tab is selected with the Transform data item expanded. The Transform data item is selected from the expanded menu.](media/pbi_transformdatamenu.png)

8. In the Power Query Editor window, select the **SalesOrderPayments (3)** table from the Queries pane. A new column is required to calculate the difference between the billing date and the actual payment data. Select the **Add Column (1)** tab and select the **Custom Column (2)** button on the toolbar menu.
...
10. In the table, right-click on the newly created **Offset (1)** field and expand the **Change Type (2)** option. Select **WholeNumber (3)** to convert this column to integer.
...
In this exercise, you retrieved database connection details, imported data into Power BI, and created a relational model. You then visualized sales and payments across various dimensions, including per date, region, and customer group, and calculated payment offsets by customer group.

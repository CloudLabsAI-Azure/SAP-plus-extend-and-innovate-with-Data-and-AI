## Exercise 6: Train a regression model to predict incoming cashflow using Azure Synapse Analytics
...
1. In Synapse Studio, select the **Data (1)** hub, then choose the **Linked (2)** tab from the center pane. Expand the **Azure Data Lake Storage Gen2 (3)** item followed by the **datalake (4)** account. Select the **sales-payment-parquet (5)** container. In the data explorer tab, right-click on the **dbo.SalesPaymentsFull.parquet (6)** and expand **New notebook (7)** then choose the **New Spark table (8)** item.
...
### Task 3: Train a new regression model for incoming cash flow (**Read-Only Task**)
...
1. In Synapse Studio, select the **Data (1)** hub, and the **Workspace (2)** tab. Expand the **Lake database (3)** section. Expand the **default** database and **Tables**. Right-click on the **salespaymentsfull** table, expand the **Machine Learning (4)** item, and choose **Train a new model (5)**. If you don't see the **Lake database** section, reload the page.
...
3. On the Train a new model (Regression) blade, select **PAYMENTDELAYINDAYS** for the **Target column** field. Ensure the Azure Machine Learning workspace is set to **sap-ml-ws** and the Apache Spark pool field is set to **SparkPoolSmall**. Select **Continue**.
...
4. Select the **Data (1)** hub from the left menu, ensure the **Workspace (2)** tab is selected in the center pane. Expand **SQL database (3)**, the **sapsynsql** database and the **Tables** items. Right-click the newly created **dbo.cashflow_prediction_tests (4)** table and expand the **Machine Learning (5)** item and select **Predict with a model (6)**.
